...
bool BehaviourTree::Save( FILE* outFile, bool swapEndian ) const
{
    ...
    if( written != write )
        return false;
    if( !m_I.Save( outFile, swapEndian ) )
        return false;
    if( !m_D.Save( outFile, swapEndian ) )
        return false;
    return true;
}
