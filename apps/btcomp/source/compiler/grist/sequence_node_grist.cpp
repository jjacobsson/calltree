...
void SequenceNodeGrist::GenerateExecutionCode( BehaviourTree* bt )
{
    ...
    int jump_to_exit_success = bt->m_I.Count();
    ...
    int exit_fail_point = bt->m_I.Count();
    bt->m_I.Push( INST__STORE_C_IN_R, E_NODE_FAIL, 0, 0 );

    //Patch the "jump to exit success"
    bt->m_I.SetA1( jump_to_exit_success, bt->m_I.Count() );
    ...
    s = exit_fail.size();
    for( int i = 0; i < s; ++i )
        bt->m_I.SetA1( exit_fail[i], exit_fail_point );
