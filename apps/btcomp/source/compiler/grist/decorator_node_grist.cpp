...
#include "decorator_node_grist.h"
#include "../decorator.h"
#include "../node.h"
#include "../node_grist.h"
#include "function_grist.h"
...
void DecoratorNodeGrist::GenerateConstructionCode( BehaviourTree* bt )
{
    ...
    if( bss_need > 0 )
    {
        m_bssPos    = bt->m_B.Push( bss_need, 4 );
    ...
    if( m_Decorator->m_Grist->m_Construct )
    {
        ...
        bt->m_I.Push( INST_STORE_PB_IN_R, 1, m_bssPos, 0 );
        // Load the register with the correct id
        bt->m_I.Push( INST_LOAD_REGISTRY, 0, (fid >> 16) & 0x0000ffff, fid & 0x0000ffff );
        // Call the decorator construction function
        bt->m_I.Push( INST_CALL_CONS_FUN, 0, 1, 2 );
    }
...
void DecoratorNodeGrist::GenerateExecutionCode( BehaviourTree* bt )
{
    ...
    {
        // Enter Debug scope
        bt->m_I.PushDebugScope( bt, m_Chaff, ACT_PRUNE );
        ...
        bt->m_I.Push( INST_CALL_PRUN_FUN, 0, 1, 2 );

        // Exit Debug scope
        bt->m_I.PopDebugScope( bt, m_Chaff, ACT_PRUNE );

        //Jump out if non-success.
        jump_out = bt->m_I.Count();
        bt->m_I.Push( INST_JABC_R_DIFF_C, 0xffffffff, E_NODE_SUCCESS, 0 );
    }
    ...
    {
        // Enter Debug scope
        bt->m_I.PushDebugScope( bt, m_Chaff, ACT_MODIFY );

        //Copy return value to bss section
        bt->m_I.Push( INST__STORE_R_IN_B, m_bssModPos, 0, 0 );
        ...
        bt->m_I.Push( INST_CALL_MODI_FUN, 0, 1, 2 );

        // Exit Debug scope
        bt->m_I.PopDebugScope( bt, m_Chaff, ACT_MODIFY );
    }

    if( jump_out != -1 )
        bt->m_I.SetA1( jump_out, bt->m_I.Count() );
...
void DecoratorNodeGrist::GenerateDestructionCode( BehaviourTree* bt )
{
    ...
    if( m_Decorator->m_Grist->m_Destruct )
    {
        ...
        bt->m_I.Push( INST_CALL_DEST_FUN, 0, 1, 2 );
    }
