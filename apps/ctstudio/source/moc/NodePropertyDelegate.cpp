...
{
  int type = index.model()->data( index, NodePropertyEditor::E_TYPE ).toInt();
  QWidget* edit = 0x0;
  switch( type )
  {
  case E_VART_INTEGER:
  ...
  const QModelIndex &index ) const
{
  ...
  case E_VART_INTEGER:
    {
      int value = index.model()->data( index, NodePropertyEditor::E_DATA ).toInt();
  ...
  case E_VART_FLOAT:
    {
      float value = index.model()->data( index, NodePropertyEditor::E_DATA ).toFloat();
  ...
  case E_VART_INTEGER:
    {
      ...
      model->setData( index, value, NodePropertyEditor::E_DATA );
    }
    break;
  case E_VART_FLOAT:
    {
