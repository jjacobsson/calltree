...
void SelectorNodeGrist::GenerateExecutionCode( BehaviourTree* bt )
{
    ...
    int exit_success_point = bt->GetCodeSection().Count();
    bt->GetCodeSection().Push( INST__STORE_C_IN_R, E_NODE_SUCCESS, 0, 0 );

    //Patch the "jump to exit fail"
    bt->GetCodeSection().SetA1( jump_to_exit_fail, bt->GetCodeSection().Count() );
    ...
    s = exit_success.size();
    for( int i = 0; i < s; ++i )
        bt->GetCodeSection().SetA1( exit_success[i], exit_success_point );
